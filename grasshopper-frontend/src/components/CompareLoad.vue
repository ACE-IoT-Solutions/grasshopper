<template>
  <div class="loader">
    ...
    <div class="container">
      <p class="alert-text">
        {{
          store.currentTask.ttl_1.replace('.ttl', '') +
          '_vs_' +
          store.currentTask.ttl_2
        }}
      </p>
      <v-progress-circular
        color="#94D8FF"
    ...
    <div style="margin-top: 10px" v-if="store.compareQueue.length > 0">
      <h5 class="title">In Queue</h5>
      <hr class="line" />
      <div v-for="item in store.compareQueue" :key="item">
        <div class="container">
          <p class="alert-text">
            {{ item.ttl_1.replace('.ttl', '') }}_vs_{{ item.ttl_2 }}
          </p>
        </div>
      </div>
    </div>
