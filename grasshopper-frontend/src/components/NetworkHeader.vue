<template>
  <div>
    <div class="network-header">
      ...
      <div
        class="buttons"
        style="margin-right: 1.5vw; gap: 20px; align-items: center"
      >
        <v-btn
          variant="plain"
          size="small"
          @click="store.setControlMenu('compare', 'Compare Graphs')"
        ...
        <v-btn
          variant="plain"
          size="small"
          @click="store.setControlMenu('delete', 'Delete')"
          >Delete</v-btn
        >
        ...
        <v-btn
          variant="outlined"
          size="small"
          @click="store.setControlMenu('setup', 'Setup')"
          color="#CDCDCD"
          >Graph Setup</v-btn
        >
        <!-- legend -->
  ...
  methods: {
    exportToFile(type) {
      if (type === 'csv') {
        ...
        excelParser().exportDataFromJSON(nodes, `${this.store.fileName}`, 'csv')
      } else if (type === 'json') {
    ...
    async exportTtl() {
      await axios
        .get(`${this.host}/api/operations/ttl_file/${this.store.fileName}`, {
          headers: {
            Accept: 'text/turtle',
          },
          responseType: 'blob',
    ...
    async exportCsv() {
      await axios
        .get(`${this.host}/api/operations/csv_export/${this.store.fileName}`, {
          headers: {
            Accept: 'text/csv',
          },
          responseType: 'blob',
        ...
        .then(response => {
          ...
          link.setAttribute(
            'download',
            `${this.store.fileName.replace('.ttl', '.csv')}`,
          )
